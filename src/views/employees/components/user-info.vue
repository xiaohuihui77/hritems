...
import { getUserDetailById, getPersonalDetail, updatePersonal, saveUserDetailById } from '@/api/user'
export default {
  data() {
    return {
  ...
  methods: {
    async getPersonalDetail() {
      ...
      if (res.data.staffPhoto.trim()) {
        this.$refs.staffImg.fileList = [{ url: res.data.staffPhoto, upload: true }]
      }
    },
    async savePersonal() {
    ...
    async saveUser() {
    //  调用父组件
      const fileList = this.$refs.staffIcon.fileList
      if (fileList.some(item => !item.upload)) {
        this.$message.warning('图片还未上传完成')
        return
      }
    ...
    async getUserDetailById() {
      ...
      if (res.data.staffPhoto.trim()) {
        this.$refs.staffIcon.fileList = [{ url: res.data.staffPhoto, upload: true }]
      }
    }
  }
}
